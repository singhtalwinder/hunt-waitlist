"""Mitratech TalentReef job extractor."""
...
class TalentReefExtractor(BaseExtractor):
    """Extractor for Mitratech TalentReef career sites."""
    ...
    async def extract(
        ...
        if html.strip().startswith("{") or html.strip().startswith("["):
            try:
                ...
                if jobs:
                    logger.info("Extracted from TalentReef JSON", job_count=len(jobs))
                    return jobs
    ...
    def _extract_from_json(self, data: dict | list) -> list[ExtractedJob]:
        """Extract jobs from TalentReef JSON response."""
        jobs = []
        ...
        if isinstance(data, list):
            ...
            job_list = data.get("jobs", []) or data.get("data", []) or data.get("positions", [])

        for job_data in job_list:
            try:
                title = job_data.get("title") or job_data.get("position_title", "")
                if not title:
                    continue
                ...
                jobs.append(
                    ExtractedJob(
                        ...
                        location=location,
                        department=job_data.get("department") or job_data.get("brand"),
                ...
                logger.debug(f"Failed to extract TalentReef job: {e}")

        return jobs

    def _extract_from_html(self, html: str, url: str) -> list[ExtractedJob]:
        """Extract jobs from TalentReef HTML page."""
        soup = BeautifulSoup(html, "lxml")
        jobs = []

        # TalentReef career site structure
        job_elements = soup.select(
            ".job-card, .job-listing, .position-card, "
            "[class*='job-row'], [data-job-id], .jobResult, .job-tile"
        )
        
        for elem in job_elements:
            try:
                title_elem = elem.select_one("a, h2, h3, h4, .job-title, .position-title")
                if not title_elem:
                    continue
                ...
                dept_elem = elem.select_one(".department, .brand")
                ...
                logger.debug(f"Failed to parse TalentReef element: {e}")

        logger.info("Extracted from TalentReef HTML", job_count=len(jobs))
        return jobs
