...
class GoHireExtractor(BaseExtractor):
    """Extractor for GoHire career pages."""
    ...
    async def extract(
        ...
        if html.strip().startswith("{") or html.strip().startswith("["):
            try:
                ...
                if jobs:
                    logger.info("Extracted from GoHire JSON", job_count=len(jobs))
                    return jobs
    ...
    def _extract_from_json(self, data: dict | list) -> list[ExtractedJob]:
        """Extract jobs from GoHire JSON response."""
        jobs = []
        ...
        if isinstance(data, list):
            ...
            job_list = data.get("jobs", []) or data.get("data", []) or data.get("vacancies", [])
        ...
        for job_data in job_list:
            try:
                ...
                logger.debug(f"Failed to extract GoHire job: {e}")

        return jobs

    def _extract_from_html(self, html: str, url: str) -> list[ExtractedJob]:
        """Extract jobs from GoHire HTML page."""
        soup = BeautifulSoup(html, "lxml")
        jobs = []

        # GoHire career page structure
        job_elements = soup.select(
            ".job-card, .job-listing, .vacancy-card, "
            "[class*='job-item'], [data-job-id], [data-vacancy-id]"
        )
        
        for elem in job_elements:
            try:
                title_elem = elem.select_one("a, h2, h3, h4, .job-title, .vacancy-title")
                if not title_elem:
                    continue
                ...
                logger.debug(f"Failed to parse GoHire element: {e}")

        logger.info("Extracted from GoHire HTML", job_count=len(jobs))
        return jobs
