...
class FolksHRExtractor(BaseExtractor):
    """Extractor for Folks HR career pages."""

    # Folks HR API base
    API_BASE = "https://api.folkshr.com/api/v2"
    ...
    async def extract(
        ...
        if html.strip().startswith("{") or html.strip().startswith("["):
            try:
                ...
                if jobs:
                    logger.info("Extracted from Folks HR JSON", job_count=len(jobs))
                    return jobs
    ...
    def _extract_from_json(self, data: dict | list) -> list[ExtractedJob]:
        """Extract jobs from Folks HR JSON response."""
        jobs = []
        ...
        for job_data in job_list:
            try:
                ...
                logger.debug(f"Failed to extract Folks HR job: {e}")

        return jobs

    def _extract_from_html(self, html: str, url: str) -> list[ExtractedJob]:
        """Extract jobs from Folks HR HTML page."""
        soup = BeautifulSoup(html, "lxml")
        jobs = []

        # Folks HR career page structure
        ...
        for elem in job_elements:
            try:
                ...
                logger.debug(f"Failed to parse Folks HR element: {e}")

        logger.info("Extracted from Folks HR HTML", job_count=len(jobs))
        return jobs
