...
async def get_pipeline_stats(db: AsyncSession = Depends(get_db)):
    ...
    # Note: 2 attempts (HTTP, then mark as custom - browser detection disabled)
    ats_stats_result = await db.execute(text('''
        SELECT 
            ...
            COUNT(*) FILTER (WHERE ats_type IS NULL AND ats_detection_attempts > 0 AND ats_detection_attempts < 2) as tried_pending,
            COUNT(*) FILTER (WHERE ats_type IS NULL AND ats_detection_attempts >= 2) as exhausted
        FROM companies
        WHERE is_active = true
    '''))
